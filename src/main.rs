...
fn main() {
    ...
    let p1 = Point {x:-4.0, y:2.0, z:-13.0};
    let p2 = Point {x:-2.0, y:-2.0, z:-7.0};
    let p3 = Point {x:2.0, y:0.0, z:-11.0};
    let sphere1 = Object::Sphere(Sphere {centre: p1, radius: 1.0, colour: RED});
    let sphere2 = Object::Sphere(Sphere {centre: p2, radius: 1.0, colour: BLUE});
    let sphere3 = Object::Sphere(Sphere {centre: p3, radius: 1.0, colour: GREEN});
    ...
    for y in 0..visualiser.screen.height() {
        for x in 0..visualiser.screen.width() {
            ...
            match scene.closest_intersection(&cam_ray) {
                Some(i) => visualiser.put_pixel(x, y, i.object.get_colour(i.location.texture_coords)),
